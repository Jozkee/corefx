...
namespace System.Text.Json
{
    ...
    {
        private static MetadataPropertyName HandleMetadataProperty(JsonSerializerOptions options, ref Utf8JsonReader reader, ref ReadStack state)
        {
            ...
            {
                // moved this to  InitEnqueued().
                //if (state.Current.IsProcessingIDictionaryConstructible)
                //{
                //    throw new JsonException("Immutable dictionaries are not supported.");
                //}

                //state.Current.LastMetaProperty = meta;

                // Should we really force this? If I remove it, the user could have multiple $ids to the same object, which doesn't seems to be an error but just a different way to handle references.
                //if (state.Current.MetadataId != null)
                //{
                //    throw new JsonException("Only one $id property must exist");
                //}
            }
